...
rwh = (116.19 + extra_h_webs) * ones(size(x_c));
lwh = (116.19 + extra_h_webs) * ones(size(x_c));
rwb = 1.27 * ones(size(x_c));
lwb = 1.27 * ones(size(x_c));
bfb = 80 * ones(size(x_c));
bfh = 0 * 1.27 * ones(size(x_c));
...
all_cs = [csTop_flange csLeft_web csRight_web csLeft_glue_connector csRight_glue_connector];
...
ybar = y_centroid;
ybot = abs(y_centroid - rw_centroid(2));
ytop = abs(tf_centroid(2) - y_centroid);
% Q at centroidal axes
A_cent1 = (total_h(cs_x) - y_centroid - tfh(cs_x) - lgch(cs_x)) * lwb(cs_x);
A_cent2 = (total_h(cs_x) - y_centroid - tfh(cs_x) - rgch(cs_x)) * rwb(cs_x);
d_cent1 = (total_h(cs_x) - y_centroid - tfh(cs_x) - rgch(cs_x)) / 2;
d_cent2 = (total_h(cs_x) - y_centroid - tfh(cs_x) - rgch(cs_x)) / 2;
Qcent = A_cent1 * d_cent1 + A_cent2 * d_cent2 + A_lgc * d_lgc + A_rgc * d_rgc + A_tf * d_tf
...
csA_total = (A_tf + A_rgc + A_lgc + A_lw + A_rw)
Volume_total = csA_total * 1260
max_shear_force_allowable = (material_shear_strength * I_yy * bcent) / (minimum_FOS * Qcent)

% Find the Distance of the Outer Design Required for a high FOS_shear
if max_shear_force_allowable > SFD(1)
    index_of_max_shear_allowable = find(abs(SFD_envelope) > max_shear_force_allowable, 1);
else 
    index_of_max_shear_allowable = find(abs(SFD_envelope) < max_shear_force_allowable, 1);
end
pos_of_max_shear_allowable = index_of_max_shear_allowable - 1;

if pos_of_max_shear_allowable < 600
    bf_Shearsupport_dist = pos_of_max_shear_allowable
else
    bf_Shearsupport_dist = L - pos_of_max_shear_allowable
end
%% 
% Finding Required Bending Moment Support

max_BM_allowable = (compressive_s * I_yy) / (minimum_FOS * (total_h(cs_x) - y_centroid) * 1000)

% Find the Distance of the Outer Design Required for a high FOS_comp
start_index_of_max_BM_allowable = find(abs(BMD_envelope) > max_BM_allowable, 1);
end_index_of_max_BM_allowable = find(abs(BMD_envelope) > max_BM_allowable, 1, 'last');


if start_index_of_max_BM_allowable < length(SFD_envelope) - end_index_of_max_BM_allowable
    dist_from_ends_for_BMSupport = start_index_of_max_BM_allowable - 1;
else
    dist_from_ends_for_BMSupport = length(SFD_envelope) - end_index_of_max_BM_allowable;
end
